 node {
    ...
    def exists=fileExists "server/package-lock.json"
    def exists2=fileExists "client/package-lock.json"
    def application_name= "app_tvi4agriculture"

        stage('Checkout') {
            git branch: 'main',
            url: 'https://github.com/spatialive/TVI4Agriculture.git'
        }
        stage('Validate') {
        ...
        stage('Build') {
                        ...
                        def status = sh(returnStatus: true, script: "cd client && ng build --stats-json --source-map=false --no-progress")
                        if (status != 0) {
                            echo "FAILED BUILD!"
        ...
        stage('Deploy container on DEV') {
                        ...
                        configFileProvider([configFile(fileId: "$File_Json_Id_APP_LAPIG_TVI4AGRICULTURE_HOMOL", targetLocation: 'container-tvi4agriculture-deploy-homol.json')]) {
                            ...
                            response = sh(script: "curl -v -X POST -H 'Content-Type: application/json' -d @container-tvi4agriculture-deploy-homol.json -s $url", returnStdout: true).trim()
                            echo response
                        }
